...
# ... (Houd je data-laden en common_steps hetzelfde) ...

# 1. Definieer de Outer Loop (voor de eerlijke evaluatie)
# We gebruiken 5 folds in de buitenkant om de robuustheid te testen.
outer_cv = KFold(n_splits=5, shuffle=True, random_state=42)

nested_results = {}

for name, (clf, params) in classifiers.items():
    print(f"Bezig met Nested CV voor {name}...")
    
    # De Pipeline
    full_pipeline = Pipeline(steps = common_steps + [('clf', clf)])
    
    # 2. De Inner Loop (GridSearchCV voor tuning)
    # Deze zit 'genest' binnen de cross_val_score
    inner_grid_search = GridSearchCV(full_pipeline, param_grid=params, cv=3, scoring='accuracy')
    
    # 3. De uitvoering van de Outer Loop
    # cross_val_score voert de buitenste splitsingen uit.
    # Voor elke outer split wordt een volledige GridSearch (inner loop) gedaan.
    nested_scores = cross_val_score(inner_grid_search, X, y, cv=outer_cv)
    ...
    print(f"{name} gemiddelde score: {nested_scores.mean():.3f} (+/- {nested_scores.std():.3f})")

# Resultaten bekijken
print("\n--- Finale Nested CV Resultaten ---")
for name, res in nested_results.items():
    print(f"{name}: Gemiddelde Accuracy = {res['mean_score']:.3f}")
# %%
