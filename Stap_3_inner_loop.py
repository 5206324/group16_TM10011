...
def inner_loop(X_train, y_train):
    ...
    classifiers = {
            ...
            'LogisticRegression': (LogisticRegression(max_iter=1000), {
                ...
                'clf__solver': ['liblinear', 'lbfgs']   # Bepaalt hoe de fout wordt geschat. 'liblinear' is goed voor kleinere datasets, 'lbfgs' is ook gangbaar
            }),
            'XGBoost': (XGBClassifier(random_state=42, use_label_encoder=False, eval_metric='logloss'), {   #XGBoost sequentially reduces errors of previous trees
                'clf__n_estimators': [50, 75, 100, 150],# Aantal bomen
                'clf__max_depth': [2, 3, 4],            # Hoe diep mag elke boom gaan? (3-5 is vaak zat voor radiomics)
                'min_child_weight': [2, 4, 6],          # Minimaal gewicht dat een leaf moet hebben om te mogen splitten
                ...
                'clf__lambda': [1, 5, 10]               # panlizes grote leaf weightts
            })
        }
